...
class graph_features(object):
    ...
    def __call__(self, data):
        ...
        # closeness
        node_closeness = nx.closeness_centrality(to_networkx(data))
        f_closeness = torch.FloatTensor([node_closeness[ii] for ii in node_closeness]).view(-1, 1)
        # clustering cofficient
        node_clustering = nx.clustering(to_networkx(data))
        f_clustering = torch.FloatTensor([node_clustering[ii] for ii in node_clustering]).view(-1, 1)
        # eigen
